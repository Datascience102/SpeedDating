...
max_data_report = 20 # can also chance in server.R
...
gender <- 1 # ie. 0 means that males make a decision; 1 means that females make a decision
...
for (x in names(CART_tree$variable.importance)) {
  y <- paste(x, labels[as.numeric(gsub("\\IV"," ",x))], sep = ": ")
  print(y)
}
```
...
df.all <- do.call(rbind, lapply(list(df1, df2, df3, df4), function(df) {
  df <- melt(df, id=1)

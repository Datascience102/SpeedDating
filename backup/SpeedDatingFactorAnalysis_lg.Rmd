---
title: "Speed dating analysis - clustering"
author: "Team A - BDA"
output:
  html_document:
...
# The Key questions
1. What are the key clusters of people going to speed dating?
2. Which clusters are the most likely to find a match?
...
1. *Part 1*: We use some of the survey questions to find key user descriptors ("factors") using *dimensionality reduction* 

2. *Part 2*: We use the selected customer descriptors to segment the speed dating community using *cluster analysis* 

3. *Part 3*: For the market segments we create, we will use *classification analysis* to classify people based on whether or not they have found a match and find what are the **match drivers per segment**. 

Finally, we will use the results of this analysis to give personal advice e.g. about brand refocusing of the personal interests, focus on appearance improvement, etc depending on the user segments and key match drivers.
...
datafile_name="data/Lukasz_SpeedDatingData_clean_2.csv"
...
# Part 1: Key Attendants Characteristics
...
factor_attributes_used = c(47,48,51:68,70:75,82:92)
